...
\newcommand{\dEsolv}{\\boldsymbol{\Delta} G_{\mathbf{solv}}}
...
\node[below right = -0.5ex and -22.0ex of image] (xaxis) {\\textbf{num. MC steps}};
\node[above left = 0.55ex and -2.0ex of image, rotate=90] (yaxis) {\\textbf{improv. over $dataset}}; 
\node[above left = -0.75ex and -31.5ex of image] (title) {\\textbf{$\dEsolv$ optimization}};
