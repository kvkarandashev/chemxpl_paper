...
# TODO : for reusing it elsewhere 3 must be replaced with something more general.
def adjusted_error(n, power):
    format_string = "{:0.3f}"
    return format_string.format(n * 10 ** (-power))
...
def opt_prob_str(quant, gap_constr):
    return opt_prob_quant_str(quant) + " (" + gap_constr + " $\gap$ constraint)"
...
diff_bias_headers = [
    ("prop. with $\\biasprop$", bias_strength[bias]) for bias in biases
]
...
with pd.option_context("max_colwidth", max_colwidth):
    ...
    for opt_prob, tab in individual_candidate_tables.items():
        ...
        cur_candidate_table[2] = multrow(
            "$\phantom{-(}" + latex_opt + "$, " + quant_measure_name[opt_prob.quant], 2
        )
        table_to_latex(
            tab,
            cur_candidate_table,
