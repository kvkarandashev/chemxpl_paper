...
def phantom_zero_padded_value_str(
    val_str, numbers_to_the_left=1, numbers_to_the_right=3, return_left_pad=False
):
    ...
    pad_to_the_left = numbers_to_the_left - cur_to_the_left
    val_str += phantom_zero_pad(numbers_to_the_right - cur_to_the_right)
    if cur_to_the_right == 0:
        val_str += "\phantom{.}"
    if return_left_pad:
        return val_str, pad_to_the_left
    else:
        val_str = phantom_zero_pad(pad_to_the_left) + val_str
        return val_str
...
def special_latex_form(
    ...
    val_str = "$" + val_str + " \pm " + RMSE_str + "$"
    if left_pad != 0:
        val_str = "$" + phantom_zero_pad(left_pad) + "$" + val_str
    return val_str
...
class EncDict:
    ...
    def latex_improve_val(self):
        ...
        return special_latex_form(
            rel_improvement,
            rel_improvement_RMSE,
            tot_length=tot_improvement_length,
...
opt_quant_val_label = "optimized quantity value"
rel_improv_label = "relative improvement"
...
opt_quant_label = (
    centered_cell("optimized"),
    "\cline{3-4}\cline{6-7}" + centered_cell("quantity"),
)
gap_constr_label = (centered_cell(r"$\Delta\epsilon$"), centered_cell("constraint"))
